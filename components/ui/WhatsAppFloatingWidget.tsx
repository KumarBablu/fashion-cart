...
export default function WhatsAppFloatingWidget() {
  ...
  return (
    <div className="fixed bottom-3 right-3 sm:bottom-4 sm:right-4 z-40 flex items-center gap-2 group animate-in fade-in duration-300">
      {/* Interactive Tooltip */}
      <div
        className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#141416]/90 backdrop-blur-md text-white text-[11px] font-semibold shadow-md border border-white/10 transition-all duration-200 pointer-events-none ${
      ...
      {/* Floating Action Button (Small, Compact & Minimalist) */}
      <button
        type="button"
        onClick={handleOpenWhatsApp}
        onMouseEnter={() => setShowTooltip(true)}
        onMouseLeave={() => setShowTooltip(false)}
        className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-md hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#25D366]/40"
