...
export default function WhatsAppFloatingWidget() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 flex items-center gap-2 group animate-in fade-in duration-300">
      {/* Interactive Tooltip */}
      <div
        className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#141416] text-white text-xs font-bold shadow-lg border border-slate-800 transition-all duration-200 pointer-events-none ${
          showTooltip ? "opacity-100 translate-x-0" : "opacity-0 translate-x-2"
        }`}
      >
        <span className="w-2 h-2 rounded-full bg-[#25D366] pulse-dot" />
      ...
      <button
        ...
        className="relative w-13 h-13 sm:w-14 sm:h-14 rounded-full bg-[#25D366] text-white flex items-center justify-center shadow-xl hover:scale-108 active:scale-95 transition-all duration-200 cursor-pointer focus:outline-none focus:ring-4 focus:ring-[#25D366]/30"
        aria-label="Connect directly with Fashion Cart on WhatsApp"
        title="Chat with Fashion Cart Stylist on WhatsApp"
      >
        {/* Pulsating background ring */}
        <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-25 pointer-events-none" />

        {/* WhatsApp Official SVG Icon */}
        <svg width="30" height="30" viewBox="0 0 24 24" fill="currentColor" className="drop-shadow-xs">
